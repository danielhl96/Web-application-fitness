...
describe('UsersController', () => {
      ...
      providers: [
        {
          ...
          useValue: {
            ...
            changeEmail: jest.fn().mockResolvedValue({ message: 'Email updated successfully' }),
            changePassword: jest
              .fn()
  ...
  describe('update Profile', () => {
    it('should  update the user profile', async () => {
      const mockUser = { id: 1, email: 'test@test.com', height: 180, weight: 75 };
      const mockData = { height: 180, weight: 95 };

      await usersController.updateProfile({ user: mockUser }, mockData);
  ...
  describe('get history', () => {
    it('should return the user history', async () => {
      const mockUser = { id: 1, email: 'test@test.com', height: 180, weight: 75 };
      const mockHistory = [
        { id: 1, user_id: 1, height: 180, weight: 75, date: new Date() },
        { id: 2, user_id: 1, height: 180, weight: 80, date: new Date() },
      ];
      usersService.getHistory = jest.fn().mockResolvedValue(mockHistory);

      const result = await usersController.getHistory({ user: mockUser });
  ...
  describe('change email', () => {
    it('should update the user email and return a success message', async () => {
      ...
      const mockBody = { email: 'test@test.com', password: 'password' };
      const mockResponse = { message: 'Email updated successfully' };
      usersService.changeEmail = jest.fn().mockResolvedValue(mockResponse);

      const result = await usersController.changeEmail({ user: mockUser }, mockBody);
  ...
  describe('change password', () => {
    it('should update the user password and return a success message', async () => {
      const mockUser = { id: 1, email: 'test@test.com' };
      const mockBody = { oldPassword: 'oldpassword', newPassword: 'newpassword' };
      ...
      const result = await usersController.changePassword({ user: mockUser }, mockBody);
  ...
  describe('should not update the user email if password is wrong', () => {
    it('should throw an error if the password is incorrect', async () => {
      const mockUser = { id: 1, email: 'das@example.com' };
      const mockBody = { email: 'newemail@example.com', password: 'wrongpassword' };
      usersService.changeEmail = jest.fn().mockRejectedValue(new Error('Password is incorrect'));

      await expect(usersController.changeEmail({ user: mockUser }, mockBody)).rejects.toThrow(
