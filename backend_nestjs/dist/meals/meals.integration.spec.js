...
describe('Meals Integration Tests (real DB)', ()=>{
    ...
    beforeAll(async ()=>{
        const module = await _testing.Test.createTestingModule({
            ...
            providers: [
                ...
                {
                    provide: _openaiservice.OpenaiService,
                    useValue: openaiMock
                }
            ]
    ...
    afterAll(async ()=>{
        ...
        await _prismaclient.prismaUser.meals.deleteMany({
            where: {
    ...
    describe('POST /meals', ()=>{
        it('should create a breakfast meal', async ()=>{
            const res = await request(app.getHttpServer()).post('/meals').send({
        ...
        it('should return 400 when required fields are missing', async ()=>{
            await request(app.getHttpServer()).post('/meals').send({
                name: 'Incomplete Meal'
            }) // missing calories, carbs, protein, fats, mealtype
            .expect(400);
        });
    ...
    describe('GET /meals', ()=>{
        ...
        let breakfastMealId;
        beforeAll(async ()=>{
            const meal = await _prismaclient.prismaUser.meals.create({
                data: {
                    user_id: parseInt(process.env.REAL_USER_ID),
        ...
        it('should return breakfast meals for today', async ()=>{
            const res = await request(app.getHttpServer()).get(`/meals?type=breakfast&date=${TODAY}`).expect(200);
            expect(Array.isArray(res.body)).toBe(true);
            expect(res.body.some((m)=>m.id === breakfastMealId)).toBe(true);
        });
        it('should return lunch meals (empty array if none)', async ()=>{
            const res = await request(app.getHttpServer()).get(`/meals?type=lunch&date=2000-01-01`).expect(200);
            expect(Array.isArray(res.body)).toBe(true);
        });
        it('should return dinner meals', async ()=>{
            const res = await request(app.getHttpServer()).get(`/meals?type=dinner&date=${TODAY}`).expect(200);
            expect(Array.isArray(res.body)).toBe(true);
        });
        it('should return snack meals', async ()=>{
            const res = await request(app.getHttpServer()).get(`/meals?type=snack&date=${TODAY}`).expect(200);
            expect(Array.isArray(res.body)).toBe(true);
        });
    ...
    describe('PUT /meals/:mealId', ()=>{
        ...
        it('should update a meal', async ()=>{
            const res = await request(app.getHttpServer()).put(`/meals/${mealId}`).send({
                name: 'Updated Meal',
                calories: 150,
                carbs: 15,
                protein: 7,
    ...
    describe('POST /meals/analyze_food_text', ()=>{
        it('should analyze food text and return nutrition data', async ()=>{
            const res = await request(app.getHttpServer()).post('/meals/analyze_food_text').send({
                text: '1 apple'
            }).expect(201);
