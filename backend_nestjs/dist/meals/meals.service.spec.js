...
describe('MealsService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(_mealsservice.MealsService);
        jest.clearAllMocks();
    });
    // ── createMeal ─────────────────────────────────────────────────────────────
    describe('createMeal', ()=>{
        it('should create a meal and return success message', async ()=>{
            ...
            expect(prismaMock.meals.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('getBreakfast', ()=>{
        it('should return breakfast meals for a user', async ()=>{
            ...
            expect(prismaMock.meals.findMany).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('calculateMeal', ()=>{
        it('should return analyzed meal data from image', async ()=>{
            ...
            openaiMock.analyzeFoodImage.mockResolvedValue(aiResult);
            const mockImage = {
                buffer: Buffer.from(''),
                mimetype: 'image/jpeg'
            };
        ...
        it('should throw if AI returns null', async ()=>{
            openaiMock.analyzeFoodImage.mockResolvedValue(null);
            await expect(service.calculateMeal({})).rejects.toThrow('Could not analyze the meal');
        });
    });
    // ── analyzeFoodText ────────────────────────────────────────────────────────
    describe('analyzeFoodText', ()=>{
        it('should return analyzed meal data from text', async ()=>{
            ...
            openaiMock.analyzeFoodText.mockResolvedValue(aiResult);
        ...
        it('should throw if AI returns null', async ()=>{
            openaiMock.analyzeFoodText.mockResolvedValue(null);
            await expect(service.analyzeFoodText('unknown food')).rejects.toThrow('Could not analyze the meal');
        });
    });
});
